...
pub use permanent_password::{
    ...
    local_permanent_password_storage_is_usable_for_auth, ENCRYPT_MAX_LEN,
};
...
#[inline]
pub fn store_path<T: serde::Serialize>(_path: PathBuf, _cfg: T) -> crate::ResultType<()> {
    Ok(())
}
...
impl Config {
    ...
    ///
    /// Returns `true` when the password is accepted or already matches the current
    /// local password. Returns `false` when changing the password is disabled or
...
impl PeerConfig {
    pub fn load(_id: &str) -> PeerConfig {
        PeerConfig::default()
    }

    pub fn store(&self, _id: &str) {}
...
impl LanPeers {
    ...
    pub fn store(_peers: &[DiscoveryPeer]) {}
...
pub fn option2bool(option: &str, value: &str) -> bool {
    if option.starts_with("enable-") {
        value != "N"
    } else if option.starts_with("allow-") || option == keys::OPTION_DIRECT_SERVER
